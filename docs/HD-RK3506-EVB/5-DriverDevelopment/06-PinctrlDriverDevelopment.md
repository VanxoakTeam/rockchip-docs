...
<img src={require('./images/06-pinctrldriverdevelopment-01.png').default} alt="image.png" style={{display: 'block', margin: '20px auto', maxWidth: '80%', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)'}}/>
...
  通常模块调⽤ pinctrl-names pinctrl-0 配置默认的IOMUX或在IOCONFIG，但不是所有的节点都可以加这两个属性，如果模块被 driver\_probe\_device 调⽤，它就可以加这两个属性。

  调试⽅法： `drivers/base/dd.c` 的pinctrl\_bind\_pins，在这⾥加打印看调⽤。